...
printenv | oc process artifactory-primary-deployment-template --ignore-unknown-parameters --param-file=- | oc replace -f -
deploy "$NAME-primary"

# Update secondary node
shutdown "$NAME-secondary"
echo "Upgrading $NAME-secondary"
printenv | oc process artifactory-secondary-deployment-template --ignore-unknown-parameters --param-file=- | oc replace -f -
